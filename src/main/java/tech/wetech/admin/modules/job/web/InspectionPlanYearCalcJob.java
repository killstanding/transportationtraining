		...
		for (int i = 0; i < inspectionPlanYears.size(); i++) {
			...
			for (int j = 0; j < inspectionPlans.size(); j++) {
				...
				if(planYearId==midPlanYearId){
					//本年度已巡检次数
					if( record.getNumberNormalDevices()!=null&&record.getNumberNormalDevices()!=0)inspectionedTimesYear=inspectionedTimesYear+1;
					//异常设备数量
					if(record.getNumberAbnormalDevices()!=null){
						abnormalNum = abnormalNum + record.getNumberAbnormalDevices();
					}
					//已报修设备数量
					if(record.getNumberReportedRepairEquipment()!=null){
			...
			for (int j = 0; j < staList.size(); j++) {
				...
				if(planYearId==midPlanYearId){
					int repairedNum = 0;
					int midRepairedNum = 0;
					if(planYear.getNumberRepairedEquipmen()!=null){
						repairedNum = planYear.getNumberRepairedEquipmen();
					}
					if(planYear.getNumberRepairedEquipmen()!=null){
						 midRepairedNum = po.getStatisticsTotal();
					}
